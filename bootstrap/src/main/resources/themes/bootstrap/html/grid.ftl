...
<tr>
[#list tag.cols as cln]
<th [#if !filterable && cln.width??] width="${cln.width}"[/#if] [#if cln.type??]class="grid-select-top" [#elseif tag.isSortable(cln)]class="grid-head-sortable text-${cln.align}" id="${cln.parameters['sort']!(tag.defaultSort(cln.property))}"[#else] class="text-${cln.align}"[/#if]>[#t/]
