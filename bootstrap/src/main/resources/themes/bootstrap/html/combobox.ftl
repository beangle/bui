  ...
  beangle.load(["chosen","bui-ajaxchosen"],function(){
    ...
    function (obj){
      ...
      jQuery.each(datas,function(i,data){
       ...
       return items;
    },{placeholder_text_single:"${tag.empty!'...'}",search_contains:true,allow_single_deselect:true,disable_search_threshold:0,as_combobox:true[#if tag.width??],width:'${tag.width}'[/#if]}
    );
  });
  [#else]
  jQuery.ajax({
